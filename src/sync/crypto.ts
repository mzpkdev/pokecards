...
// of the network so it unit-tests directly.
//
...
// version suffixes give headroom to rotate the derivation without colliding with existing data. The
// "pokecards" namespace is deliberate: a pokecards code can never derive a questline blob's id/key
// and vice versa, even from an identical code string.
const HKDF_SALT = "pokecards-sync-hkdf-salt-v1"
const INFO_ID = "pokecards-sync-id-v1"
const INFO_KEY = "pokecards-sync-key-v1"
...
// wrong version, failed auth tag from a wrong key or tampering). Callers treat null as "reject, change
// nothing."
