...
// A collection blob is kilobytes; cap well above that but small enough that KV/logs stay cheap.
...
export async function readBlob(env: Env, id: string): Promise<Response> {
    ...
    const { value, metadata } = await env.POKECARDS.getWithMetadata<{ version: number }>(id)
...
export async function writeBlob(env: Env, id: string, request: Request): Promise<Response> {
    ...
    await env.POKECARDS.put(id, body, { metadata: { version } })
